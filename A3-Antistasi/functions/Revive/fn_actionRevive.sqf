...
if ((([_cured] call A3A_fnc_fatalWound)) and (not("Medikit" in (items _medicX)))) exitWith
	{
	if (_player) then {hint format ["%1 has a serious injury, you need a MediKit to revive them",name _cured]};
	if (_inPlayerGroup) then {_medicX groupChat format ["%1 has a serious injury, you need a MediKit to revive them",name _cured]};
	_healed
	};	
	...
	else
	{
		...
		if (([_medicX] call A3A_fnc_canFight) and !(_medicX getVariable ["cancelRevive",false]) and (_medicX == vehicle _medicX) and (alive _cured)) then
		{
			if (_cured getVariable ["INCAPACITATED",false]) then
			{
				...
				if (_player) then
				{
				[1,_medicX,"revive"] remoteExec ["a3c_fnc_updateStat",_medicX];
				};
