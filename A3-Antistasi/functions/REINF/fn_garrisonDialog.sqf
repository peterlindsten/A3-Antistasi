...
if (getMarkerPos _nearX distance _positionTel > 40) exitWith {["Garrison", "You must click near a marked zone"] call A3A_fnc_customHint; _nul=CreateDialog "a3c_build_menu";};

if (not(sidesX getVariable [_nearX,sideUnknown] == teamPlayer)) exitWith {["Garrison", format ["That zone does not belong to %1",nameTeamPlayer]] call A3A_fnc_customHint; _nul=CreateDialog "a3c_build_menu";};
if ([_positionX,500] call A3A_fnc_enemyNearCheck) exitWith {["Garrison", "You cannot manage this garrison while there are enemies nearby"] call A3A_fnc_customHint; _nul=CreateDialog "a3c_build_menu"};
...
if (_typeX == "rem") then
	{
	if ((count _garrison == 0) and !(_nearX in outpostsFIA)) exitWith {["Garrison", "The place has no garrisoned troops to remove"] call A3A_fnc_customHint; _nul=CreateDialog "a3c_build_menu";};
	_costs = 0;
	_hr = 0;
	{
	...
	_nul=CreateDialog "a3c_build_menu";
	}
else
	{
	...
	closeDialog 0;
	_nul=CreateDialog "a3c_garrison_recruit";
	sleep 1;
