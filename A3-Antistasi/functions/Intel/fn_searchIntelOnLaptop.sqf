...
if(_isTrap) exitWith
{
    _intel setObjectTextureGlobal [0, "\antistasi_client\resource\image\laptop_die.paa"];
    {
...
_intel setObjectTextureGlobal [0, "\antistasi_client\resource\image\laptop_downloading.paa"];
...
while {_pointSum <= _neededPoints} do
{
    ...
    if(!_actionNeeded) then
    {
        ...
        if(random 1000 < _errorChance) then
        {
            ...
            _picturePath = format ["\antistasi_client\resource\image\laptop_%1.paa", _picturePath];
            ...
            [
                ...
                [
                    _actionText,
                    {
                        ...
                        (_this select 0) setObjectTextureGlobal [0, "\antistasi_client\resource\image\laptop_downloading.paa"];
...
{
    _intel setObjectTextureGlobal [0, "\antistasi_client\resource\image\laptop_complete.paa"];
